...
const MainComment: React.FC<MainCommentProps> = ({user, index, postId}) => {
    const commentData = COMMENTDATA.filter((data) => data.post_id === postId)
    const bottomSheetModel = useBottomModalStore()
  return (
    commentData.length !== 0 ? (
        <View style={{ flexDirection: 'row', justifyContent: 'center' }} key={index}>
            ...
            >
                ...
                <TouchableOpacity onPress={() => {bottomSheetModel.onOpen(<PostPreview user={user} index={index} postId={postId}/>)}}>
